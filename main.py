...
def main(args):
    ...
    SAVE_PATH = '%s/ckpt/%s_%s_%s_b%s_step%s'%(args.workdir, args.dataset, args.actor_model, args.reward_model, args.batch_size, args.train_steps)
    if args.do_sample:
    ...
    prompt_max_len = 36    # forced prompt to be 32 token; add 4 for safety
    ...
    for epoch in range(1):
        ...
        for idx, (batch, cont_batch) in tqdm(enumerate(zip(train_prompt_loader, train_cont_loader))):
            if args.train_steps >= 0 and tot_step >= args.train_steps:
                break
            ...
            cont_input_ids = cont_batch['input_ids'][:,1:].to(device)  # ignore the first BOS token
            ...
            cont_attention_mask[:,:prompt_length] = prompt_attention_mask

            with torch.no_grad():
                output = actor_model(seq, attention_mask=seq_attention_mask)
                output_ref = ref_model(seq, attention_mask=seq_attention_mask)
                reward_score = reward_model.forward_value(seq, seq_attention_mask, prompt_length=prompt_length)['chosen_end_scores'].detach()
            ...
            if exp_dataset is not None and len(exp_dataset) >= 1:
                ...
                if args.paraphraser != '':
                    ...
                    # Train the reward model with paraphrased output.
                    inp_list = []
                    eot_flags = []
                ...
                if args.other_llm != '':
                    ...
                    other_attention_mask[:,:prompt_length] = prompt_attention_mask
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='c4', choices=['c4'])
