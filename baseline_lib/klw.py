...
from .klw_utils import normalization_strategy_lookup
...
#        return self.model.generate(logits_processor=LogitsProcessorList([watermark_processor]), *args, **kwargs)

class ActorWrapper:
    def __init__(self, model, tokenizer, delta=1.0):
        watermark_processor = WatermarkLogitsProcessor(vocab=list(tokenizer.get_vocab().values()),
                                                   gamma=0.25,
                                                   delta=delta,
                                                   seeding_scheme="simple_1")
        #self.module = ModelWrapper(model, watermark_processor)
        model.generate_func = model.generate
        model.generate = lambda *args, **kwargs: model.generate_func(logits_processor=LogitsProcessorList([watermark_processor]), *args, **kwargs)
        self.module = model
...
class RMWrapper:
    ...
    def __call__(self, llm_ids, llm_mask, human_ids, human_mask):
        ...
        for i in range(len(llm_ids)):
            tokenized_text = llm_ids[i][llm_mask[i]!=0]
            detect_output = self.watermark_detector.detect(tokenized_text=tokenized_text, return_prediction=False, return_scores=True)
            output['chosen_mean_scores'].append(detect_output['z_score'])
        for i in range(len(human_ids)):
            tokenized_text = human_ids[i][human_mask[i]!=0]
            detect_output = self.watermark_detector.detect(tokenized_text=tokenized_text, return_prediction=False, return_scores=True)
            output['rejected_mean_scores'].append(detect_output['z_score'])
